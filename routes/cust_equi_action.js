...
exports.contentupload = function(req, res) {
    ...
    function getcutomerdata(callback) {
        var query = "SELECT enid, contact_email,contact_mobileno FROM `tbl_login` WHERE loginid = (SELECT cp_email FROM tbl_customers WHERE custid  = ?)"
    ...
    function savedata(customerdata, callback) {
        ...
        var query = "INSERT INTO `tbl_tickets` (`coid`, `txnrefno`, `custid`, `contactno`, `contactemail`,`prod_brid`, `prodid`,`brandname`,`prodcatname`,`machinename`, `batchdate`,`issuedetails`, `serialno`, `modelno`,`reqtype`, `imgurl1`, `imgurl2`, `imgurl3`, `imgurl4`, `imgurl5`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"

        var params = [coid, txnrefno, customerdata[0].enid, customerdata[0].contact_mobileno, customerdata[0].contact_email, post.prod_brid, post.product, brandname, prodcatname, post.machinename, post.batchdate, post.description, post.serialno, post.modelno,post.reqtype];
