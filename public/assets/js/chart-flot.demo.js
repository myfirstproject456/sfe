...
var handleStackedChart = function () {
    ...
    var ticksLabel = [
        [0, "Customer1"], [1, "Customer2"], [2, "Customer3"], [3, "Customer4"],
...
var handleInteractivePieChart = function () {
	...
	if ($('#interactive-pie-chart').length !== 0) {
        var data = [];
        var name =['sof','apical','comp1','comp2','comp3'];
        var series = 4;
        var colorArray = [purple, dark, grey];
        for( var i = 0; i<series; i++)
        {
            data[i] = { label:name[i] , data: Math.floor(Math.random()*100)+1, color: colorArray[i]};
        }
